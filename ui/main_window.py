...
class EurocodeReaderApp(ctk.CTk):
    ...
    def _build_menu_bar(self) -> None:
        ...
        for sequence, action in (
            ...
            ("<Control-o>", self.load_workspace),
            ("<Control-s>", self.save_workspace),
            ("<Control-S>", self.save_workspace_as),      # Ctrl+Shift+S
    ...
    def manage_global_equations(self) -> None:
        """Open the equation library on its own.

        Deliberately independent of everything else: no PDF has to be
        indexed, no workflow open and no node selected. An engineer can sit
        down and type out their standard expressions first.
        """
        manage_equations(self, self.equation_library)
        self.set_status(
            f"Equation library: {len(self.equation_library)} saved "
    ...
    def new_workspace(self) -> None:
        """Start a blank workspace, offering to save any unsaved chart first."""
        if not self._confirm_discard():
            return
        self.flowchart_view.adopt_chart(None, None)
        self.workspace_path = None
        self.workspace_name = "Untitled workspace"
        self.workspace_dirty = False
        self.search_view.refresh_document_list()
        self._refresh_workspace_bar()
        self.set_status(
            "New workspace. Your indexed PDFs are still here - only the "
            f"workflow was cleared. {DISCLAIMER}"
        )
